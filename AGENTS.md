...
- **aislop** — AI-slop/code-quality scanner. `.aislop/config.yml` is committed and turns the `formatting` rule off: aislop bundles its own Biome with a wider line width than `biome.json` sets, so with both on, the two reformatted the same files in opposite directions. Biome remains the only formatter, enforced by the pre-commit hook and the blocking `check` CI job. No `.aislop/rules.yaml` yet. `.gitignore` un-ignores `.aislop/` before re-excluding its contents, because a machine-wide ignore would otherwise stop git descending into it. Runs on staged files in pre-commit (`aislop ci --staged`) and as a Claude Code PostToolUse hook (see your global `AISLOP.md` instructions — treat its findings as blocking). CI runs it over changed files on PRs and the whole repo on `main` (`mise run slop:changes` / `slop:check`), report-only.
