...
- **`ai-hygiene.yml`** — runs `aislop` (code slop) and `ctxharness` (agent-doc drift) via the `mise run slop:check` / `slop:changes` / `ctx` tasks and posts a sticky PR comment plus job summary. Advisory only; never fails the build. The `ctxharness` check currently has nothing to verify against — there's no `.ctxharness.yml` yet, so it reports "config not found" until one exists with real assertions (see [Known drift](AGENTS.md#instructions) in `AGENTS.md`). Not wired into the pre-commit/pre-push hooks for the same reason.
